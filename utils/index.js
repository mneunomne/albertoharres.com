...
export const IMAGE_SCALE = 35
export const CANVAS_OUT_MARGIN = 200
...
export const getImageSizeOnScreen = (img_src) => new Promise((res, rej) => {
  var canvasHeight = getCanvasHeight(window);
  var vFOV = (CAMERA_FOV * Math.PI) / 180;
  var visibleHeight = 2 * Math.tan(vFOV / 2) * CAMERA_DISTANCE; // visible height
  // load image
...
export const getMobileCameraDistance = (imageObjectSize, _window) => new Promise((res, rej) => {
  ...
  if (img.complete) {
    ...
    var cameraDistance = (canvasHeight / 2) / Math.tan(vFOV / 2);
  ...
  /*
  var cameraDistance = (canvasWidth / 2) / Math.tan(vFOV / 2);
  console.log("calculateCameraDistance", cameraDistance)
...
export const getContentMargin = (_window) => {
